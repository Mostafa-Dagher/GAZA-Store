...
@Component({
  selector: 'app-product-by-category-id',
  templateUrl: './product-by-category-id.component.html',
  styleUrls: ['./product-by-category-id.component.css']
})


export class ProductByCategoryIdComponent implements OnInit {
  products: any[] = [];
  searchterm: any;
  ...
  ngOnInit(): void {
    ...
    this._ActivatedRoute.paramMap.subscribe({
      next: (params) => {
        this.productByCategoryId = params.get('id');
      }
    });

    this._ProductsService.getProductByCaticoryid(this.productByCategoryId, 50).subscribe({
      next: (products) => {
